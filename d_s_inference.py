...
def plot_mars(pred_mars, prompt_batch):
    labels = ["Pos x", "Pos y", "Pos z", "Size x", "Size y", "Size z", "Rot x", "Rot y", "Rot z", "Opacity"]
    for sample_id in range(pred_mars.shape[0]):
        ...
        plt.title('10 Sequences of Length 60')
        plt.xlabel('Index')
        plt.ylabel('Value')
        plt.legend()
        plt.savefig(f'{prompt_batch[sample_id]}_{sample_id}.png')
...
start_state = torch.tensor([0,0,0,1,1,1,0,0,1,1], dtype = torch.float32).unsqueeze(0).repeat(len(prompt_batch),1).to(device)
xt_noise = torch.randn(size=[B, 60, len(valid_idx)], device=device)
mars = sampler(xt_noise, img_batch, prompt_batch, start_state[:,config.valid_idx])
