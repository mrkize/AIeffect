...
def generate_json_from_keyframe_data_v1(keyframe_data):
    ...
    for i, layer_data in enumerate(layer_keyframe_data):
        layer_frames = layer_data[
            "total_frames"] if "total_frames" in layer_data else total_frames
        layer_size = layer_data[
            "origin_size"] if "origin_size" in layer_data else None
        layer_info = mars_generator_layered.layer_info_template.copy()
        layer_info["id"] = layer_data["id"] if "id" in layer_data else str(i)
        layer_info["nm"] = layer_data[
            "name"] if "name" in layer_data else "layer_" + str(i)
        layer_info["type"] = layer_data["type"]
        layer_info["p"] = [0, 0]
        layer_info["anchor"] = layer_data["anchor"]
        if "img" in layer_data:
            layer_info["img"] = global_tools.upload_file(layer_data["img"], i)
            layer_info["img_type"] = "path"
        if "parent" in layer_data:
            layer_info["parent"] = layer_data["parent"]
        layer_info["pkf"] = layer_info["okf"] = layer_info["skf"] = layer_info[
            "rxkf"] = layer_info["rykf"] = layer_info["rzkf"] = False
        ...
        opacity_kf_new = [
            round(opacity_kf[j] * visiable_kf[j] * 10000) / 10000
            for j in range(layer_frames)
        ]
        while opacity_kf_new[
                start_frame] == 0 and start_frame < layer_frames - 1:
            start_frame += 1
        ...
        if check_keypoint_all_same(opacity_kf_new):
            ...
            layer_info["oa"] = True
            layer_info["o"] = [
                mars_curve_fitter.fit_hermite_curves_from_points(
                    opacity_kf_new)
            ]
        ...
        for k in range(3):
            scale_k[k] = [
                round(scale_kf[j][k] / layer_size[k] * 10000) / 10000
                for j in range(start_frame, end_frame)
            ]
        ...
        if check_scale_all_same:
            ...
            if np.allclose(scale_k[0], scale_k[1], 1e-3, 1e-3):
                ...
                for k in range(3):
                    layer_info["s"][
                        k] = mars_curve_fitter.fit_hermite_curves_from_points(
                            scale_k[k])
        ...
        rotation_kf_x = [
            rotation_kf[j][0] for j in range(start_frame, end_frame)
        ]
        rotation_kf_y = [
            rotation_kf[j][1] for j in range(start_frame, end_frame)
        ]
        rotation_kf_z = [
            rotation_kf[j][2] for j in range(start_frame, end_frame)
        ]
        ...
        if check_keypoint_all_same(rotation_kf_x):
            ...
            layer_info["rxa"] = True
            layer_info["rx"] = [
                mars_curve_fitter.fit_hermite_curves_from_points(rotation_kf_x)
            ]
        ...
        if check_keypoint_all_same(rotation_kf_y):
            ...
            layer_info["rya"] = True
            layer_info["ry"] = [
                mars_curve_fitter.fit_hermite_curves_from_points(rotation_kf_y)
            ]
        ...
        if check_keypoint_all_same(rotation_kf_z):
            ...
            layer_info["rza"] = True
            layer_info["rz"] = [
                mars_curve_fitter.fit_hermite_curves_from_points(rotation_kf_z)
            ]
        ...
        layer_info["pp"] = []
        layer_info["cp"] = []
        pa, path_fitter_results = mars_curve_fitter.fit_bezier_path_from_points(
            position_kf[start_frame:end_frame])
        # print(pa, path_fitter_results)
        if pa:
            layer_info["pa"] = True
