...
d_trainer = GaussianDiffusionTrainer(config)
# DiTmodel = DiT.from_pretrained("/ossfs/workspace/aieffects/save_ckp/diffusionDiT/final_model",config=config)
# d_trainer.model = DiTmodel
# d_trainer = GaussianDiffusionTrainer.from_pretrained("/ossfs/workspace/aieffects/save_ckp/diffusionCT/results/checkpoint-4400",config=config)
...
trainer = Trainer(
    ...
    data_collator=dataset.diffusion_collate,
)
...
# 保存最终模型
trainer.model.model.save_pretrained(args.final)
